...
> "What does Instagram do? It tries to understandand what I like and keep giving me that. Sometimes it adds new content in between to test if I like it or not, otherwise it keeps giving me content that I have liked in the past"

Recommendation systems consist of three components, a) User pool, b) Item pool, and c) Interactions. The general gist of the system being: recommend items to the users that the users might like. Typically this happens using the interactions of the users. 

What about the users who have not interacted yet, who are just dipping the toes into the platform or just haven't gotten active enough to learn anything from their behavior? These users are called cold and sparse users. They're like the new kids on the block, and traditional recommendation systems often struggle to figure out what they might like because, well, they don't have much to go on.
...
What do I mean about treatment? Good question. We will come back to this in a bit.

## Step 1 - Analysis

You will, of course, be following the [guide to recommender systems in prod](https://knhash.in/blog/recommender-system-in-prod). I will not have otherwise. Unless you are experienced, then procees without looking at the above linked guide.
...
- Fetch and clean user meta data for our dense users
- Cluster the user meta
...
| Cluster | DPI       | Tier      | Gender        | Price  | Preference Categories ( by priority)               |
|---------|-----------|-----------|---------------|--------|-----------------------------------------------------------|
...
## Step 2 - Productionising

How do get from here to ranking content for the users? It is time to talk treatment.

Dense user's have models running for them that leverage their interactions to personalize content better. We could use one of the models' output prediction pattern. A typical way to do this would be:

- Find the cluster that a sparse user, A, belongs to; call it X
- Find the dense user at the centre of the cluster, c(X) = Y
- Find the predictions we are going to send for the dense user, P(Y)
- Send _these_ predicitons to the sparse user, P(A) -> P(Y)

These predictions would need to run for the (one) dense user in realtime multiple times, i.e., whenever the sparse users request for predictions. If the input conditions don't change for the dense user then we can save the dense user weights in a low latency cache and use it through the period until the input conditions change.
...
The results of the A/B experiment demonstrate the effectiveness of the clustering-based recommendation system in improving user engagement and interaction rates. The next steps were to dig into the various levers and try to ekk out more wins. But this, as it stands, is in itself a success.
